...
//   type: "track" | "project";
//   data: TrackData;
//   onTodoToggle?: (lessonId: string,isComplet:boolean) => void;
// }

// const ProgressCard = ({ type, data, onTodoToggle }: ProgressCardProps) => {
//   // Flatten lessons with unique ids
//   const flattenLessons = (): { id: string; task: string; completed: boolean }[] => {
//     return data.items.flatMap((item, itemIndex) =>
//       item.lessons.map((lesson, lessonIndex) => ({
//         id: `${itemIndex}-${lessonIndex}`,
//         task: lesson.lessonName,
//         completed: lesson.isComplet,
//       }))
//     );
//   };

//   const [todos, setTodos] = useState(flattenLessons());

//   useEffect(() => {
//     setTodos(flattenLessons());
//   }, [data]);
...
//       )
//     );
//     const clickedTask = todos.find((todo) => todo.id === todoId);
//     if (clickedTask) onTodoToggle?.(clickedTask.task,clickedTask.completed);
//   };
...
//           {type === "track" ? "Track" : "Graduation project"}
//         </h2>

//         {/* Track/Project Name with Progress */}
//         <div className="space-y-2">
//           <div className="flex justify-between items-center">
//             <span className="text-lg">{data.trackName}</span>
//             <span className="text-[#0A2647]">{progress}%</span>
//           </div>
//           <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
//             <div
...
//               <ul className="space-y-3">
//                 {item.lessons.map((lesson, lessonIndex) => {
//                   const todoId = `${index}-${lessonIndex}`;
//                   const completed = todos.find((t) => t.id === todoId)?.completed;
//                   return (
//                     <li
//                       key={todoId}
//                       className="flex items-center gap-3 cursor-pointer group"
